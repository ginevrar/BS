...
# ----end sed conc
#----- PLOT----------------
svg('dato_mode_hg_mehg.svg')
#dev.new(width=7, height=7)
par(mfrow=c(1,2),cex.axis=1.2, cex.lab=1.3, 
    cex.main=1.4, mar=c(4.5,3.5,1,0))
...
segments(x1+sd1,y-epsilon,x1+sd1,y+epsilon, col='#5716a2')
par(new=TRUE)
plot(disshg_pM,prof, type="b",pch=23,  lwd=1,cex=2.5,
     ylim=c(17.2,10.9),  bty='n', xlim=c(0, 5),lty=3,
     xlab="Hg concentrations (pM)", col="#b3db25", bg='#b3db2577', 
     ylab='')
mtext(expression(paste(sigma[theta]*'  (kg/m'^3*')')),
      2, line=2,at=14,col="black", cex=1.2)

p<-c(21,23)
legend(4,11, pch=p,legend=c("Data", 
                              'Model'),
       col=c("#5716a2", '#b3db25'), 
...
par(mar=c(4.5,1,1,1.5), cex.axis=1.2, cex.lab=1.3, cex.main=1.4 )
plot(medie_mehg_pM, prof,  bty='n',lwd=1.3, cex=1.3,
     ylim=c(17.2,10.9), lty=1,yaxt='n',
     xlim=c(0, 1), pch=22,
...
plot(dissMehg_pM, prof, type="b",pch=23,  lwd=1.3,cex=2.5,
     ...
     xlab="MeHg concentrations (pM)", col="#b3db25", yaxt='n',
