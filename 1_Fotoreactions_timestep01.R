...
fotored_kmols_y_media<-tapply(fotored_kmols_y[1:73365],rep(1:(length(fotored_kmols_y[1:73365])/365), each = 365), mean)
mean(fotored_kmols_y_media)
#FOTOSSIDAZIONE
fotox_kmols_y_media<-tapply(fotox_kmols_y[1:73365],rep(1:(length(fotox_kmols_y[1:73365])/365), each = 365), mean)
fotox_kmols_y_media<-as.numeric(fotox_kmols_y_media); mean(fotox_kmols_y_media)
#FOTODEG
fotodem_kmols_y_media<-tapply(fotodem_kmols_y[1:73365],rep(1:(length(fotodem_kmols_y[1:73365])/365), each = 365), mean)
...
output_kmol_y<-cbind(fotox_kmols_y, fotored_kmols_y, 
                     ...
                     fotox_1_s, fotodem_1_s)

fotored_1_s_media<-tapply(fotored_1_s[1:73365],rep(1:(length(fotored_1_s[1:73365])/365), each = 365), mean)
fotored_1_s_media<-as.numeric(fotored_1_s_media)


fotox_1_s_media<-tapply(fotox_1_s[1:73365],rep(1:(length(fotox_1_s[1:73365])/365), each = 365), mean)                          
fotox_1_s_media<-as.numeric(fotox_1_s_media)

fotodem_1_s_media<-tapply(fotodem_1_s[1:73365],rep(1:(length(fotodem_1_s[1:73365])/365), each = 365), mean)                          
fotodem_1_s_media<-as.numeric(fotodem_1_s_media)

Hg0_pM_media1<-tapply(Hg0_pM1[1:73365],rep(1:(length(Hg0_pM1[1:73365])/365), each = 365), mean)
HgII_pM_media<-tapply(HgII_pM[1:73365],rep(1:(length(HgII_pM[1:73365])/365), each = 365), mean)
mehg_pM_media<-tapply(mehg_pM[1:73365],rep(1:(length(mehg_pM[1:73365])/365), each = 365), mean)
...
Volat1_kmol_y_media<-as.numeric(Volat1_kmol_y_media)

plot(Volat1_kmol_y_media, type="l"); 
...
write.csv(volat1_kmol_y, file="volat_kmoly_.csv", row.names=F)
