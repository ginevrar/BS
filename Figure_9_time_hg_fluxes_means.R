...
out_medinew1 <-read.table("water_input_output1_2050", header=TRUE, sep=",")
str(out_medinew1) 
# input
inf_new1<-out_medinew1$hgT_inflow_kmol_y_media
diff_new1<-out_medinew1$diffusion_kmol_y_media
#output
vol_new1<-out_medinew1$evasione_kmol_y
outf_new1<-out_medinew1$hgT_outflow_kmol_y_media
pHgdepo_new1<-out_medinew1$depo_Phg_kmol_y_media
...
SE_vol<-sd_vol/sqrt(13)*2
SE_vol[164]
vol_min<-rowMeans(v_2013[3:201,])-(SE_vol[3:201])
vol_max<-rowMeans(v_2013[3:201,])+(SE_vol[3:201])
vol_poly<-c(vol_min,rev(vol_max))
evasione_kmol_y<-rowMeans(v_2013)

sd_diff<-apply(diff_2013, 1, FUN=sd)
SE_diff<-sd_diff/sqrt(13)*0.14
diff_min<-rowMeans(diff_2013[3:201,])-SE_diff[3:201]
diff_max<-rowMeans(diff_2013[3:201,])+SE_diff[3:201]
diff_poly<-c(diff_min,rev(diff_max))
diffusion_kmol_y<-rowMeans(diff_2013)
SE_b1<-sd(b1_2013)/sqrt(13)

sd_b2<-apply(b2_2013, 1, FUN=sd)
SE_b2<-sd_b2/sqrt(13)*2.6
b2_min<-rowMeans(b2_2013[3:201,])-SE_b2[3:201]
b2_max<-rowMeans(b2_2013[3:201,])+SE_b2[3:201]
y<-seq(3:201)
yy<-c(y,rev(y))
...
CI_inf_min<-rowMeans(inf_2013[3:201,])-SE_inf[3:201]
CI_inf_max<-rowMeans(inf_2013[3:201,])+SE_inf[3:201]
inf_poly<-c(CI_inf_min,rev(CI_inf_max))

sd_outf<-apply(outf_2013, 1, FUN=sd)
SE_outf<-sd_outf/sqrt(13)*1.1
CI_outf_min<-rowMeans(outf_2013[3:201,])-SE_outf[3:201]
CI_outf_max<-rowMeans(outf_2013[3:201,])+SE_outf[3:201]
outf_poly<-c(CI_outf_min,rev(CI_outf_max))

sd_pHgdepo<-apply(pHgdepo_2013, 1, FUN=sd)
SE_pHgdepo<-sd_pHgdepo/sqrt(13)
CI_pHgdepo_min<-rowMeans(pHgdepo_2013[3:201,])-SE_pHgdepo[3:201]
CI_pHgdepo_max<-rowMeans(pHgdepo_2013[3:201,])+SE_pHgdepo[3:201]
phgdep_poly<-c(CI_pHgdepo_min,rev(CI_pHgdepo_max))
depo_Phg_kmol_y<-rowMeans(pHgdepo_2013)
riv_2013
sd_riv<-apply(riv_2013, 1, FUN=sd)
SE_riv<-sd_riv/sqrt(13)*1.46
SE_riv[164]
CI_riv_min<-rowMeans(riv_2013[3:201,])-SE_riv[3:201]
CI_riv_max<-rowMeans(riv_2013[3:201,])+SE_riv[3:201]
...
tiff('Fig.9A_Hgver_means.tiff', height=25, width=23, units='cm', 
     compression="lzw", res=300)
par(mfrow=c(1,1), mar=c(5,5,4,1), bty='n') 
plot(atm_hg_kmol_y[3:201], col="cyan3", type="l",  ylim=c(-30,50), 
...
par(new=TRUE)
plot(river_hg_kmol_y[3:201], col="chartreuse3", type="l", 
     xaxt='n',yaxt='n',ylim=c(-30,50), 
     ylab= " ",xlab= " ",  lwd=1)
polygon(yy,riv_poly,col='#65cc0044',border = '#65cc0044')
par(new=TRUE)
plot(hgT_inflow_kmol_y[3:201], col="darkblue", type="l", 
     xaxt='n',yaxt='n',
     ylim=c(-30,50), ylab= " ", xlab= " ",  lwd=2)
par(new=TRUE)
plot(diffusion_kmol_y[3:201], col="#B8860B", type="l", 
     xaxt='n',yaxt='n',
     ylim=c(-30,50), ylab= " ", xlab= " ",  lwd=2)
polygon(yy,diff_poly,col='#B8860B44',border = '#B8860B44')
par(new=TRUE)
plot(Input_terms[3:201], col="black", type="l", lty=2, 
     xaxt='n',yaxt='n',
     ylim=c(-30,50), ylab= " ", xlab= " ",  lwd=1)
polygon(yy,int_poly,col='#00000044',border = '#00000044')

par(new=TRUE)
plot(-evasione_kmol_y[3:201], col="cyan4", type="l", xlab= " ", 
     ylim=c(-30,50), xaxt='n',yaxt='n',
     main=" ", ylab= " ",  lwd=1)
polygon(yy,-vol_poly,col='#007f7f44',border = '#007f7f44')
par(new=TRUE)
plot(-depo_Phg_kmol_y[3:201], col="orange", type="l", xlab= " ", 
     ylim=c(-30,50), xaxt='n',yaxt='n',
     ylab= " ",  lwd=1)
polygon(yy,-phgdep_poly,col='#FFA50044',border = '#FFA50044')
par(new=TRUE)
plot(-hgT_outflow_kmol_y[3:201], col="darkblue", type="l", 
     ylim=c(-30,50),xlab= " ",xaxt='n',yaxt='n',
     ylab= " ",  lwd=2)
polygon(yy,-outf_poly,col='#00000044',border = '#00000044')

par(new=TRUE)
plot(-Output_terms[3:201], col="black", type="l", lty=2, 
     ylim=c(-30,50), xaxt='n',yaxt='n',
     xlab= " ", ylab= "",  lwd=1)
...
text(30,40,'Input', col=1,cex=2.2)
text(30,-26,'Output', col=1,cex=2.2)
mtext(expression(paste('kmol y'^-1)), 2, at= 2, line=2.6, cex=2)

text(123,27,'River \n load', cex=1.9,col='chartreuse4')
text(90,-27,'Deposition to \n the sediment', cex=1.9,col='darkorange')

text(185,-3.2,'Outflow',cex=1.9, col='darkblue')
text(185,3.4,'Inflow',cex=1.9, col='darkblue')
text(130,3.5,'Sediment Diffusion ',cex=1.7, col='darkgoldenrod')

text(130,12,'Atmospheric \n deposition',col='cyan4', cex=1.9)
text(130,-8.5,'Evasion',col='cyan4', cex=1.9)
