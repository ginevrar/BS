...
rect(0, y-h, pos2, y+h, col=c('#fdd49e','#fdd49e',"lightcyan2",
                              ...
                              "lightcyan2"))
text(2,.8, expression("MeHg"[P]*~"Deposition \n to the Sediment"))
text(-2,3.9, expression("MeHg"[P]*~"Deposition \n to the Sediment"))
text(-1.50,5.7, expression("MeHg"[T]*~"Outflow"))
text(2,3.9,  expression("MeHg"[T]*~"Rivers Load"))
text(1.5,5.7, expression("MeHg"[T]*~"Inflow"))
text(2.1,4.9, expression("Net methylated MeHg"))
text(1.8,2.8, expression("MeHg"[D]*~"Diffusion"))
text(-1.8,1.8, expression("MeHg"[D]*~"Diffusion"))
text(-1.8,.8, expression("MeHg"[P]*~"Burial"))

title("MeHg Budget in the Black Sea [kmol/y]")
par(cex.axis=1, mex=0.5)
axis(1, at=ticks, labels=(ticks), pos=0)
tw <- 1.5*strwidth("neg")
segments(-5, 2.2, 5, 2.2, lty=2, col="black") 
text(3.9,5.7, "Water", cex=1.2, font=4, col='cyan4')
text(4.1,1.9, "Sediment", cex=1.2, font=4, col='darkorange1')
