...
mehg<-uc5_hr$MeHg
  ...
  pperc<-mehg/hg*100
perc<-data.frame(pperc,uc5_hr$Depth)
...
tiff('HRprof.tiff', res=400,  width = 3000, height = 3000, units = "px")
par(mfrow=c(1,3),oma=c(7,6,5,6), mar=c(0.1,0.1,0.1,0.1),bty='n' )
...
mtext(expression(paste('MeHg'[D])),side=1, at=-0.25,line=0.2, col='#e7298a', cex=1.2 )
...
axis(1, at=axp, tick=T,line=2.9, las=1, cex.axis=1.3, col='darkorchid4')
axis(1, at=axp2, tick=T,labels =F,line=2.9, tcl=-.2, las=1, cex.axis=1.3, col='darkorchid4')
mtext(expression(paste('MeHg%')),side=1, at=-25,line=3.6 ,col="darkorchid4", cex=1.2)
...
axis(1, at=axfe, tick=T,line=2.9, las=1, cex.axis=1.3, col='black')
axis(1, at=axfe2, tick=T,labels=F,line=2.9, tcl=-.2,las=1, cex.axis=1.3, col='black')
mtext(expression(paste('Fe'[D]*'(nM)')),side=1, at=500,line=2.9 ,col="#cd6600", cex=1.2)

par(new=T)
ss<-c(14.2,14.9, 15.6, 15.9, 16.2, 16.4,16.5)
axis(4, at= dd, labels =ss,tick=T, las=1, cex.axis=1.3, col='black')
mtext(expression(paste(sigma[theta]*'  (kg m'^-3*')')), side=4, cex=1.3, line=4.2)
Mn<-uc52$Mn/1000
par(new=T)
...
axmn<-seq(0,5)
axmn2<-seq(.5,4.5)
axis(3, at=axmn, tick=T, las=1, cex.axis=1.3, col='black')
axis(3, at=axmn2, labels =F,tick=T, las=1, cex.axis=1.3, col='black', tcl=-.2)
mtext(expression(paste('Mn'[D]*'(然)')),side=3,  at=2.5,line=2.3 , col='darkgoldenrod1', cex=1.2)
par(new=T)
...
axis(1, at=axph, tick=T, las=1, cex.axis=1.3, col='black')
axis(1, at=axph2, tick=T,labels=F, las=1, tcl=-.2, cex.axis=1.3, col='black')
mtext(expression(paste('PO'[4]^3*'(然)')),side=1,  at=10,line=1.3 , col='darkgreen', cex=1.2)
